...
c.lof=(message,farbe=false)=>{if(farbe){c.log("%c"+message,"color:"+farbe);}
else{c.log(message)}};selectBoxSpace={selectBoxPlanet:[],poblate:()=>{if(d.querySelector('.SelectBox')){var selectBoxes=d.querySelectorAll('.SelectBox');selectBoxes.forEach(selectBox=>{selectBoxSpace.selectBoxPlanet.push(new SelectBox(selectBox))});}},}
class SelectBox{constructor(element){this.element=element;this.list=element.querySelector('.selectBoxList');this.button=element.querySelector('.selectBoxButton');this.className=element.className;this.id=element.id;this.config={attributes:true,childList:true,characterData:true}}
select(a,selectBoxId,current){if(!!a){d.querySelector(selectBoxId).classList.add('alt')}
...
class GrowUp{constructor(element){this.element=element;this.className=element.className;this.target=element.dataset.target;this.step=50;this.timeDuration=2500;this.current=parseFloat(element.innerHTML);this.config={attributes:true,childList:true,characterData:true}
this.init();}
