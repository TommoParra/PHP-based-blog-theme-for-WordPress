...
function ajax_paginator_2( $query ){
	...
	$pagination = '<nav class="pagination flex_no_wrap flex_row_center_between" role="navigation">';
	...
	if ($current_page != 1)
		...
		$pagination.= '<button class="button button_sm button_black button_prev flex_no_wrap flex_row_center_between pagination_link" data-pagination="prev" data-cycle-container="'.$query->query_vars['cycle'].'"><span class="pagination_line line line_sm line_white line_to_left"></span> <span class="pagination_txt_hideable">Anterior</span></button>';
	...
	for($i = $first_link_in_the_middle; $i <= $last_link_in_the_middle; $i++) {
		if($i == $current_page) {
			...
			// $pagination .= '<a class="pagination_link" data-cycle-container="'.$query->query_vars['cycle'].'" data-pagination="'.$i.'">'.$i.'</a>';
		}
	}
	...
	if ($current_page != $last_link_in_the_middle )
		...
		$pagination.= '<button class="button button_sm button_black button_next flex_no_wrap flex_row_center_between pagination_link" data-pagination="next" data-cycle-container="'.$query->query_vars['cycle'].'"><span class="pagination_txt_hideable">Siguiente</span> <span class="pagination_line line line_sm line_white line_to_right"></span></button>';
...
add_action( 'wp_ajax_tp_get_tags', 'tp_get_tags' );
// We allow non-logged in users to access our pagination
add_action( 'wp_ajax_nopriv_tp_get_tags', 'tp_get_tags' );
function tp_get_tags() {
	$respuesta = '';
	$search = $_POST['search'];
	...
	foreach ( $term_query->terms as $term ) {
			...
			$respuesta .= "<span class='nearch_tag'>$term->name</span>";
	}
	echo $respuesta;
	exit();
}
